...
contract CitNFT is ERC721URIStorage, Ownable, Whitelistable {
  ...
  function _mint(
    ...
    address _to
  ) internal onlyNoneHolder(msg.sender) onlyUnlocked returns (uint256) {
    ...
    require(
      cJPY.allowance(_to, address(this)) >= userBalance,
   ...
   * for now we have commented it since we do not want the token to be transferred.
   */
  ...
  ) public onlyOwner hasTokenId(tokenId) {
  ...
  function transferFrom(
    ...
    address _to,
    uint256 _tokenId
  ) public override(ERC721) forbidden {}
